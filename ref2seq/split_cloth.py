...
with open(in_fp, 'r') as f:
    for i, line in tqdm(enumerate(f), desc = "1st pass of reviews"):
        ...
        if (user, text) in pairs:
            continue
        pairs.add((user, text)) # only keep one edu per (u,i) pair
        user_count[user] += 1
        item_count[item] += 1
...
for user, items in tqdm(interactions.items(), desc='make interactions'):
    ...
    # vote in train/dev/test are also sorted by time
    # x = [item, rating, ts, text]. 
    # append [user, item, text]. 
